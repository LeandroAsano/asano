...
- **Bloque 1 hecho (2026-07-24):** migrado a **Expo Router** (rutas en `app/`, entry `expo-router/entry`), design tokens de la paleta cálida en `src/theme/tokens.ts`, y React Navigation removido. La pantalla Home vive en `app/index.tsx`, el layout raíz en `app/_layout.tsx`.
- **Pendiente (Bloque 2):** persistencia sigue con **AsyncStorage** (`src/storage/`, modelo `Habit` simple en `src/types/`) — migrar a **SQLite + Drizzle** y al modelo de datos del doc 03 (check-in, acción mínima, ajustes, rachas flexibles). Las fuentes Montserrat/Inter todavía no están cargadas (tokens `fonts` en `undefined`, usan la del sistema).
