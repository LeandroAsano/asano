...
- **Pendiente:** cargar fuentes Montserrat/Inter (tokens `fonts` en `undefined`). Ampliar el modelo del doc 03 (habit_schedules, adjustments, programs) en los próximos bloques. `minimalAction` ya existe en el schema pero el flujo de creación real (Ciclo ASA) y el check-in tri-estado llegan después. El onboarding no tiene forma de repetirse desde Ajustes todavía (no existe pantalla de Ajustes).
