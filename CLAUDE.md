...
- **Pendiente:** ampliar el modelo del doc 03 (habit_schedules, adjustments, programs). `minimalAction` ya existe en el schema pero el flujo de creación real (Ciclo ASA) y el check-in tri-estado con ajuste de ASA llegan después. No hay pantalla de Ajustes todavía (por eso el onboarding no se puede repetir).
